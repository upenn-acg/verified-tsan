  ...
  - rewrite app_assoc in Hcon; exploit consistent_app_SC; eauto; clarify.
    ...
    + destruct (instr_result t i' (G' t) v') as [((((?, ?), ?), ?), ?)|]
      ...
      * exploit v_not_C; eauto; clarify.
        ...
        inversion Hi1; subst.
        rewrite bounded_spawn in *; clarify.
      * destruct Haccess as [? | [? | ?]]; clarify.
        { contradiction Hsafei1; apply R_meta; auto. }
        { contradiction Hsafei1; apply W_meta; auto. }
      * exploit v_not_X; try apply Haccess; clarify.
...
Fixpoint not_lock (x : var) i :=
  let fix not_lock_list l :=
    match l with
    | [] => True
    | i :: rest => not_lock x i /\ not_lock_list rest
    end in
  match i with
  | Lock l => l <> x
  | Unlock l => l <> x
  | Spawn _ li => not_lock_list li
  | _ => True
  end.

Fixpoint not_lock_list x li :=
  match li with
  | [] => True
  | i :: rest => not_lock x i /\ not_lock_list x rest
  end.
Lemma not_lock_list_iff : forall x li,
  (fix not_lock_list l :=
    match l with
    | [] => True
    | i :: rest => not_lock x i /\ not_lock_list rest
...
Fixpoint good_var x (P : state) :=
  match P with
  | [] => True
  | (_, li) :: rest => not_lock_list x li /\ good_var x rest
  end.

Lemma good_var_app : forall x P1 P2, good_var x (P1 ++ P2) <->
  good_var x P1 /\ good_var x P2.
Proof.
  induction P1; [split|]; clarify.
  destruct a; rewrite IHP1; split; clarify.
Qed.

Lemma good_var_step : forall x P G t o c P' G' (Hlock : good_var x P)
  (Hstep : exec P G t o c (Some P') G'), good_var x P'.
Proof.
  intros; inversion Hstep; clarify; rewrite good_var_app in *; clarify.
  rewrite (not_lock_list_iff x) in *; auto.
Qed.

Corollary good_var_steps : forall x P G o c P' G' (Hlock : good_var x P)
  (Hsteps : exec_star (Some P) G o c (Some P') G'), good_var x P'.
Proof.
  intros; remember (Some P) as P1; remember (Some P') as P2;
    generalize dependent P; induction Hsteps; clarify.
...
Lemma protect_v' : forall P0 (Hsafe : safe_locs P0)
  ...
  lo lc P' G' (Hsteps' : exec_star (Some P) G lo lc P' G')
  t (Hno_lock : Forall (fun a => a <> Acq t (X + fst v)) lc)
  (Hcon : consistent (m ++ lc0 ++ lc))
  a (Ha : In a lc) (Ht : thread_of a = t) (Haccess : v_access v a),
  can_read (m ++ lc0) (X + fst v, 0) (S t).
Proof.
  intros.
  remember (Some P) as P1; generalize dependent P; rewrite exec_rev in Hsteps'; 
    induction Hsteps'; clarify.
  ...
  exploit good_var_steps; eauto; intro.
  exploit protect_v; eauto.
  - intro; subst.
    exploit in_split; eauto; clarify.
    rewrite good_var_app in *; clarify.
  - intro; subst.
    exploit in_split; eauto; clarify.
    rewrite good_var_app in *; clarify.
  - unfold v_access in Haccess; setoid_rewrite Hloc in Haccess; auto.
...
Corollary protect_held : forall P0 (Hsafe : safe_locs P0)
  (Ht : Forall (fun e => fst e < zt) P0) P0' (Hsim : state_sim P0 P0')
  (Hdistinct : distinct P0') G0 lo0 lc0 P G
  (Hsteps : exec_star (Some P0') G0 lo0 lc0 (Some P) G)
  v (Hv : fst v < zv) (Hmeta : ~meta_loc v)
...
Lemma t_steps_lock1 : forall P G t lo lc P1 G1 m li
  (Hdistinct : distinct P)
  (Hin : In (t, instrument_instr (Lock m) t ++ li) P)
  (Hsteps : t_steps P G t (length (instrument_instr (Lock m) t)) lo
                    lc (Some P1) G1) (Hin' : In (t, li) P1),
  exists vs1 vs2, filter (fun c => beq (thread_of c) t) lc =
...
Proof.
  ...
  intro.
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; eauto; [eapply exec_minus_exec|]; eauto; intro.
  exploit t_steps_max_vc; eauto.
  intros (vs1 & vs2 & Hlc1).
  generalize (exec_minus_ops Hminus0); intro Hall0.
  exists vs1, vs2; clarify.
  rewrite filter_app, (filter_none _ Hall0), Hlc1; simpl; eauto.
Qed.

Corollary t_steps_lock : forall P G t lo lc P1 G1 m li
  (Hdistinct : distinct P)
  (Hin : In (t, instrument_instr (Lock m) t ++ li) P)
  (Hsteps : t_steps P G t (length (instrument_instr (Lock m) t) - 1) lo
                    lc (Some P1) G1) o1 c1 P2 G2
  (Hstep : exec P1 G1 t o1 c1 (Some P2) G2) (Hin' : In (t, li) P2),
  exists vs1 vs2, filter (fun c => beq (thread_of c) t) (lc ++ opt_to_list c1) =
    Acq t m :: mops_max_vc (L' + m) (C' + t) vs1 vs2 t zt.
Proof.
  intros.
  eapply t_steps_lock1; eauto.
  exploit t_steps_extend'; eauto.
  { eapply exec_step'; eauto.
    * apply exec_refl.
    * rewrite app_nil_r; auto. }
  { unfold instrument_instr; rewrite app_length, lock_handler_len; simpl; omega.
  }
Qed.
...
Lemma t_steps_inc_vc' : forall t u P' G' tgt tmp P G lo lc li
  (Hdistinct : distinct P)
  (Hin : In (t, inc_vc u tgt tmp ++ li) P)
  (Hsteps : t_steps P G t (length (inc_vc t tgt tmp)) lo lc
     (Some P') G'),
  exists v, filter (fun c => beq (thread_of c) t) lc =
    mops_inc_vc' tgt v t u.
Proof.
  intros; simpl in Hsteps.
  destruct Hsteps as (o0 & c0 & P0 & G0 & Hstep0 & lo1 & lc1 & [P1|] & G1 &
    lo2 & lc2 & Hminus0 & Hstep & ?); [|clarify].
...
Lemma t_steps_wait1 : forall P G t lo lc P1 G1 u li
  (Hdistinct : distinct P)
  (Hin : In (t, instrument_instr (Wait u) t ++ li) P)
  (Hsteps : t_steps P G t (length (instrument_instr (Wait u) t)) lo
                    lc (Some P1) G1) (Hin' : In (t, li) P1),
  exists vs1 vs2 v,
...
Proof.
  ...
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; eauto; [eapply exec_minus_exec|]; eauto; intro.
  unfold wait_handler in Hrest1, H; rewrite app_length in Hrest1.
  exploit t_steps_plus; eauto.
  intros (lo3 & lc3 & P3 & G3 & lo4 & lc4 & Hcheck & Hrest' & ? & ?); subst.
  clear Hrest1.
  rewrite <- app_assoc in H.
  exploit t_steps_max_vc; try apply Hcheck; eauto; intros (vs1 & vs2 & Hlc3).
  exploit t_steps_length'; try apply Hcheck; eauto; intro Hin3.
  exploit distinct_steps; try eapply t_steps_exec; eauto; intro.
  exploit t_steps_inc_vc'; try apply Hin3; eauto; intros (v & Hlc4).
  exists vs1, vs2, v; simpl.
  repeat rewrite filter_app; rewrite Hlc3, Hlc4.
  generalize (exec_minus_ops Hminus0); intro Hall0.
  rewrite (filter_none _ Hall0); auto.
Qed.
...
(*Lemma suffix_thread : forall P P1 P2 (Hsuffix : state_suffix P P1)
  ...
  (Hin : In (t, li) P2),
  (exists li0 n, In (t, li0) P /\
   n < length (instrument_instr (hd (Assign 0 (I 0)) li0) t) /\
   In (t, skipn n (instrument li0 t)) P1 /\
   exists n', n + n' < length (instrument_instr (hd (Assign 0 (I 0)) li0) t) /\
     li = skipn (n + n') (instrument li0 t)) \/
  (exists t0 n i rest n', In (t0, skipn n (instrument_instr i t0) ++ rest) P1 /\
     n < length (instrument_instr i t0) /\ In (t0, skipn n' rest) P2).
Proof.
  intros.
  exploit in_steps_rev2; eauto; intros [(li1 & ?) | Hspawn].
  - exploit Forall2_in2; eauto; intros ((? & li0) & ? & ? & n & ? & Heq);
      clarify.
    exploit app_eq_skip; eauto; clarify.
    destruct (le_gt_dec (length (instrument_instr (hd (Assign 0 (I 0)) li0) t0))
      (n + length li1)).
    + right; destruct li0; clarify.
      { rewrite skipn_nil in *.
        destruct n; [|omega].
        destruct li1; clarify; omega. }
      rewrite Heq, skipn_app, not_le_minus_0 in H; [|omega].
      simpl in H; repeat eexists; eauto.
      rewrite skipn_skipn, skipn_app, skipn_all in Hin; [eauto | omega].
    + left; rewrite Heq in *; exists li0, n; clarify.
      repeat eexists; eauto.
      apply skipn_skipn.
  - right; destruct Hspawn as (Hout & ? & ? & ? & ? & ? & t' & li' & rest & li2 
      & ? & ? & ? & ? & ? & ? & Hspawn & Hsteps' & ?); clarify.
    exploit Forall2_in2; eauto; intros ((t1, li1) & ? & ? & n & ? & Heq).
    destruct li1; clarify.
    { rewrite skipn_nil in *; destruct li2; clarify. }
    ...
    { rewrite split_app in Heq.
      destruct (le_gt_dec (length (li2 ++ [Spawn t' li']))
        (length (skipn n (instrument_instr i t1)))).
      * exploit app_eq_inv_ge; eauto; intros (? & ? & Heq1).
        exploit spawn_in_handler'.
        { rewrite <- skipn_nth; setoid_rewrite Heq1.
          rewrite <- app_assoc; simpl; apply nth_error_split. }
        intros (? & ? & ? & Hlen); subst.
        symmetry in Heq1; generalize (app_eq_skip _ _ Heq1).
        rewrite app_length, skipn_skipn, skipn_all; [exists 0; clarify|].
        rewrite plus_assoc, (plus_comm n), Hlen; simpl; omega.
...
Corollary suffix_thread1 : forall P P1 P2 (Hsuffix : state_suffix P P1)
  (Hdistinct : distinct P1) G1 lo lc G2
  (Hsteps : exec_star (Some P1) G1 lo lc (Some P2) G2),
  Forall (fun (e : tid * list instr) => let (t, li) := e in
    exists li0, In (t, li0) P /\ exists n,
    n < length (instrument_instr (hd (Assign 0 (I 0)) li0) t) /\
    li = skipn n (instrument li0 t)) P2 \/ exists t n i rest n',
    In (t, skipn n (instrument_instr i t) ++ rest) P1 /\
  ...
  assert (Forall (fun (e : tid * list instr) => let (t, li) := e in
    (exists li0 n, In (t, li0) P /\
     ...
     exists n', n + n' < length (instrument_instr (hd (Assign 0 (I 0)) li0) t)
      /\ li = skipn (n + n') (instrument li0 t)) \/
    (exists t0 n i rest n', In (t0, skipn n (instrument_instr i t0) ++ rest) P1 
      /\ n < length (instrument_instr i t0) /\ In (t0, skipn n' rest) P2)) P').
  { rewrite Forall_forall; intros (t, li); subst; eapply suffix_thread; eauto. }
...
(*Lemma suffix_next_thread : forall P P1 P2 (Hsuffix : state_suffix P P1)
  (Hdistinct : distinct P1) G1 lo lc G2
  (Hsteps : exec_star (Some P1) G1 lo lc (Some P2) G2),
  state_suffix P P2 \/ exists t n i rest n',
    In (t, skipn n (instrument_instr i t) ++ rest) P1 /\
    n < length (instrument_instr i t) /\ In (t, skipn n' rest) P2.
Proof.
  intros.
  exploit suffix_thread1; eauto; clarify; left.
  ...
  - exploit Forall2_app_inv_r; eauto; intros (? & ? & ? & Hrest & ?); subst.
    ...
    rewrite <- (distinct_suffix Hsuffix) in Hdistinct.
    exploit distinct_in.
    { eauto. }
    { eauto. }
    { rewrite in_app; clarify. }
  ...
  - constructor; auto.
    do 2 eexists; eauto.
    exploit Forall2_in1; eauto; intros ((?, ?) & Hin1 & ? & n1 & ?); clarify.
    exploit exec_mono; eauto.
    { rewrite in_app; clarify. }
    intros (n2 & Hskip); rewrite skipn_skipn in Hskip.
    destruct n'; [exploit skip_cons_neq; eauto; contradiction|].
    exists n'; split; [omega|].
    erewrite (skipn_n n'); eauto.
    assert (n1 + n2 = n').
    { assert (length (i :: skipn (S n') (instrument li0 t)) =
        length (skipn (n1 + n2) (instrument li0 t))) by (rewrite Hskip; auto).
...
Proof.
  repeat intro.
  repeat rewrite <- app_assoc.
  unfold mem_ext in H; rewrite H; reflexivity.
Qed.  
...
Proof.
  intros ??? Hext1 Hext2 ops.
  rewrite (Hext1 ops); auto.
Qed.

(* !! This should be incorporated into the above and into exec_result. *)
Lemma exec_next' : forall P G t o c P' G' (Hexec : exec P G t o c P' G')
  (Hdistinct : distinct P) Pa Pb i li (Hin : P = Pa ++ (t, i :: li) :: Pb),
...
Lemma t_steps_hb_check_n : forall t P' G' C1 C2 z P G lo lc li
  (Hdistinct : distinct P)
  (Hin : In (t, hb_check C1 C2 z tmp1 tmp2 ++ li) P)
  n (Hsteps : t_steps P G t n lo lc P' G')
  (Hn : n <= length (hb_check C1 C2 z tmp1 tmp2)),
  (exists vs1 vs2 n', n' <= length (mops_hb_check C1 C2 vs1 vs2 z t) /\
     filter (fun c => beq (thread_of c) t) lc =
     firstn n' (mops_hb_check C1 C2 vs1 vs2 z t)) /\
   forall a, a <> tmp1 -> a <> tmp2 -> G' t a = G t a.
Proof.
  induction z; [clarify | intros].
  { destruct n; [clarify | exfalso; omega].
    exists [], [], 0; auto. }
  destruct n; [clarify|].
  { exists [], [], 0; auto. }
  unfold hb_check in Hin; fold hb_check in Hin; rewrite <- app_assoc in Hin.
  simpl in Hsteps.
  destruct Hsteps as (o0 & c0 & P0 & G0 & Hstep0 & lo1 & lc1 & P1 & G1 &
    lo2 & lc2 & Hminus0 & Hstep); clarify.
  exploit in_split; eauto; clarify.
  exploit exec_next'; eauto; intros (v1 & ?); clarify.
  exploit exec_minus_ops; eauto; intro Hops1.
  destruct P1; clarify.
  exploit exec_other_threads; try apply Hminus0; try apply split_in; intro.
  rewrite filter_app, (filter_none _ Hops1).
  destruct n; [clarify|].
  { split; [exists [v1], [v1], 1; clarify|].
    intros; erewrite exec_minus_env; [|eauto].
    apply upd_old; auto. }
  simpl in Hstep1.
  destruct Hstep1 as (o1 & c1 & P2 & G2 & Hstep1 & lo3 & lc3 & P3 & G3 &
    lo4 & lc4 & Hminus1 & Hstep).
  exploit in_split; eauto; clarify.
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; try eapply exec_minus_exec; eauto; intro.
  exploit exec_next'; eauto; intros (v2 & ?); clarify.
  exploit exec_minus_ops; eauto; intro Hops3.
  destruct P3; clarify.
  exploit exec_other_threads; try apply Hminus1; try apply split_in; intro.
  rewrite filter_app, (filter_none _ Hops3).
  destruct n; [clarify|].
  { split; [exists [v1], [v2], 2; clarify|].
    intros; erewrite exec_minus_env; [|eauto].
    rewrite upd_old; auto.
    erewrite exec_minus_env; [|eauto].
    apply upd_old; auto. }
  simpl in Hstep2.
  destruct Hstep2 as (o2 & c2 & P4 & G4 & ? & lo5 & lc5 & P5 & G5 &
    lo6 & lc6 & Hminus2 & Hstep).
  exploit in_split; eauto; clarify.
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; try eapply exec_minus_exec; eauto; intro.
  exploit exec_next'; eauto; clarify.
  destruct (le_dec (G3 t tmp1) (G3 t tmp2)); clarify.
  exploit exec_minus_ops; eauto; intro Hops5.
  destruct P5; clarify.
  exploit exec_other_threads; try apply Hminus2; try apply split_in; intro.
  rewrite filter_app, (filter_none _ Hops5).
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; try eapply exec_minus_exec; eauto; intro.
  exploit IHz; eauto.
  { omega. }
  intros ((vs1 & vs2 & n' & ? & Hlcr) & HG).
  split.
  - exists (v1 :: vs1), (v2 :: vs2), (S (S n')); simpl.
    erewrite exec_minus_env in l; eauto.
    rewrite upd_same, upd_old in l; auto.
    erewrite exec_minus_env in l; eauto.
    rewrite upd_same, <- leb_le in l; clarify.
    split; [omega|].
    rewrite Hlcr; auto.
  - intros; rewrite HG; auto.
    erewrite exec_minus_env; [|eauto].
    ...
    erewrite exec_minus_env; [|eauto].
    apply upd_old; auto.
  - rewrite (filter_none _ Hops5); auto.
    split; [exists [v1], [v2], 2; simpl; split; [omega | auto]|].
    intros; erewrite exec_minus_env; [|eauto].
    erewrite exec_minus_env; [|eauto].
    rewrite upd_old; auto.
    erewrite exec_minus_env; [|eauto].
  ...
  - inversion Hminus2; clarify.
    split; [exists [v1], [v2], 2; simpl; split; [omega | auto]|].
    intros; erewrite exec_minus_env; [|apply Hminus1].
    rewrite upd_old; auto.
    erewrite exec_minus_env; [|eauto].
    apply upd_old; auto.
  - rewrite (filter_none _ Hops3); auto.
    split; [exists [v1], [v2], 2; simpl; split; [omega | auto]|].
    intros; erewrite exec_minus_env; [|apply Hminus1].
    rewrite upd_old; auto.
    erewrite exec_minus_env; [|eauto].
    apply upd_old; auto.
  - rewrite (filter_none _ Hops1); auto.
    split; [exists [v1], [v1], 1; simpl; split; [omega | auto]|].
    intros; erewrite exec_minus_env; [|apply Hminus0].
    apply upd_old; auto.
Qed.

Lemma t_steps_plus' : forall t n1 n2 P G lo lc P' G'
  (Ht_steps : t_steps P G t (n1 + n2) lo lc P' G'),
  exists lo1 lc1 P1 G1 lo2 lc2, t_steps P G t n1 lo1 lc1 P1 G1 /\
    match P1 with Some P1 => t_steps P1 G1 t n2 lo2 lc2 P' G' /\
      lo = lo1 ++ lo2 /\ lc = lc1 ++ lc2
    | None => P' = None /\ G' = G1 /\ lo = lo1 /\ lc = lc1 /\ lo2 = [] /\
              lc2 = [] end.
Proof.
  induction n1; simpl; intros.
  - repeat eexists; auto.
...
Lemma t_steps_move_n : forall t P' G' C1 C2 P G lo lc li
  (Hdistinct : distinct P)
  (Hin : In (t, move C1 C2 tmp1 ++ li) P)
  n (Hsteps : t_steps P G t n lo lc P' G')
  (Hn : n <= length (move C1 C2 tmp1)),
  (exists v n', n' <= length (mops_move C1 C2 t v) /\
     filter (fun c => beq (thread_of c) t) lc = firstn n' (mops_move C1 C2 t v))
  /\ forall a, a <> tmp1 -> G' t a = G t a.
Proof.
  intros.
  destruct n; [clarify|].
  { exists 0, 0; auto. }
  unfold move in *; simpl in *.
  destruct Hsteps as (o0 & c0 & P0 & G0 & Hstep0 & lo1 & lc1 & P1 & G1 &
    lo2 & lc2 & Hminus0 & Hstep); clarify.
  exploit in_split; eauto; clarify.
  ...
  erewrite exec_minus_env; [|eauto].
  rewrite upd_same.
  split; [exists v, 2; clarify|].
  intros; erewrite exec_minus_env; [|eauto].
  erewrite exec_minus_env; [|eauto].
  apply upd_old; auto.
  - rewrite (filter_none _ Hops3).
    erewrite exec_minus_env; [|eauto].
    rewrite upd_same.
    split; [exists v, 2; clarify|].
    intros; erewrite exec_minus_env; [|eauto].
    erewrite exec_minus_env; [|eauto].
    apply upd_old; auto.
  - rewrite (filter_none _ Hops1).
    split; [exists v, 1; clarify|].
    intros; erewrite exec_minus_env; [|eauto].
    apply upd_old; auto.
Qed.

Lemma t_steps_move : forall t P' G' C1 C2 P G lo lc li
  (Hdistinct : distinct P)
  (Hin : In (t, move C1 C2 tmp1 ++ li) P)
  (Hsteps : t_steps P G t (length (move C1 C2 tmp1)) lo lc (Some P') G'),
  (exists v, filter (fun c => beq (thread_of c) t) lc = mops_move C1 C2 t v) /\
   forall a, a <> tmp1 -> G' t a = G t a.
Proof.
  intros.
  unfold move in *; simpl in *.
...
Proof.
  ...
  rewrite filter_app, (filter_none _ Hops1).
  exploit exec_other_threads; try apply Hminus0; try apply split_in; intro.
  exploit in_split; eauto; intros (P'a & P'b & ?); subst.
  repeat rewrite <- app_assoc in *.
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; try eapply exec_minus_exec; eauto; intro.
  destruct (le_gt_dec n (length (hb_check (W' + x) (C' + t) zt tmp1 tmp2))).
  { exploit t_steps_hb_check_n; eauto; intros ((vs1 & vs2 & n' & Hle & Hlcr) &
      ?).
    exists vs1, vs2, 0, 0, (S n'); simpl.
    rewrite <- Nat.sub_0_le in Hle; rewrite Hlcr, firstn_app, Hle.
    split; [|rewrite app_nil_r; auto].
    rewrite app_length; omega. }
  exploit le_plus_minus.
  { apply lt_le_weak; eauto. }
  intro Heq; rewrite Heq in *; clear Heq.
  exploit t_steps_plus'; eauto.
  intros (lo3 & lc3 & P3 & G3 & lo4 & lc4 & Hcheck & Hrest').
  destruct P3; clarify.
  exploit t_steps_hb_check; try apply Hcheck; try apply split_in; auto.
  intros ((vs1 & vs2 & Hlc3) & HG3); exists vs1, vs2; rewrite filter_app, Hlc3.
  exploit t_steps_length'; try apply Hcheck; try apply split_in; auto.
  intro; exploit in_split; eauto; intros (P3a & P3b & ?); subst.
  exploit distinct_steps; eauto.
  { eapply t_steps_exec; eauto. }
  intro; destruct (le_gt_dec (n - length (hb_check (W' + x) (C' + t) zt tmp1
    tmp2)) (length (move (C' + t, t) (R' + x, t) tmp1))).
  { exploit t_steps_move_n; eauto; intros ((v & n' & Hle & Hlcr) & ?).
    exists v, 0, (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      n')); simpl.
    rewrite firstn_app, firstn_length', minus_plus; [|omega].
    split.
    + repeat rewrite app_length; simpl.
      clear - Hle.
      apply le_n_S, plus_le_compat_l, le_plus_trans; auto.
    + rewrite <- Nat.sub_0_le in Hle.
      rewrite Hlcr, firstn_app; setoid_rewrite Hle.
      rewrite app_nil_r; auto. }
  exploit le_plus_minus.
  { apply lt_le_weak; eauto. }
  intro Heq; rewrite Heq in *; clear Heq.
  exploit t_steps_plus'; eauto.
  intros (lo5 & lc5 & P5 & G5 & lo6 & lc6 & Hmove & Hrest2).
  destruct P5; clarify.
  exploit t_steps_move; try apply Hmove; try apply split_in; auto.
  intros ((v & Hlc5) & HG5); exists v; rewrite filter_app, Hlc5.
  exploit t_steps_length'; try apply Hmove; try apply split_in; auto.
  intro; exploit in_split; eauto; intros (P5a & P5b & ?); subst.
  exploit distinct_steps; eauto.
  { eapply t_steps_exec; eauto. }
  intro; destruct (n - length (hb_check (W' + x) (C' + t) zt tmp1 tmp2) -
    length (move (C' + t, t) (R' + x, t) tmp1)) eqn: Hn'; simpl in *.
  { clarify.
    exists 0, (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      length (mops_move (C' + t, t) (R' + x, t) t v))); simpl.
    split; [repeat rewrite app_length; omega|].
    rewrite firstn_app, firstn_length', minus_plus, firstn_app, firstn_length,
      minus_diag; [auto | omega]. }
  destruct Hrest21 as (o1 & c1 & P4 & G4 & Hstep1 & lo7 & lc7 & P7 & G7 &
    lo8 & lc8 & Hminus1 & Hrest2).
  exploit exec_next'; eauto; intros (v' & ?); exists v'; clarify.
  exploit exec_minus_ops; eauto; intro Hops7.
  destruct P7; clarify.
  rewrite filter_app, (filter_none _ Hops7).
  exploit exec_other_threads; try apply Hminus1; try apply split_in; intro.
  exploit in_split; eauto; clarify.
  destruct n0; simpl in *.
  { clarify.
    exists (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      (length (mops_move (C' + t, t) (R' + x, t) t v) + 1))); simpl.
    split; [repeat rewrite app_length; simpl; omega|].
    rewrite firstn_app, firstn_length', minus_plus, firstn_app, firstn_length',
      minus_plus; auto; omega. }
  destruct Hrest21 as (o2 & c2 & P8 & G8 & Hstep2 & lo9 & lc9 & P9 & G9 &
    lo10 & lc10 & Hminus2 & Hrest2).
  exploit distinct_step; eauto; intro.
  exploit distinct_steps; try eapply exec_minus_exec; eauto; intro.
  exploit exec_next'; eauto; clarify.
  exploit exec_minus_ops; eauto; intro Hops9.
  destruct P9; clarify.
  rewrite filter_app, (filter_none _ Hops9).
  exploit exec_other_threads; try apply Hminus2; try apply split_in; intro.
  destruct n0; [clarify|].
  exists (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      (length (mops_move (C' + t, t) (R' + x, t) t v) + 2))); simpl.
  split; [repeat rewrite app_length; simpl; omega|].
  rewrite firstn_app, firstn_length', minus_plus, firstn_app, firstn_length',
    minus_plus; auto; omega.
  - repeat rewrite app_length in Hn; simpl in Hn.
    exfalso; clear - Hn.
    apply le_S_n, plus_le_reg_l, plus_le_reg_l in Hn; omega.
  - rewrite (filter_none _ Hops9).
    exists (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      (length (mops_move (C' + t, t) (R' + x, t) t v) + 2))); simpl.
    split; [repeat rewrite app_length; simpl; omega|].
    rewrite firstn_app, firstn_length', minus_plus, firstn_app, firstn_length',
      minus_plus; auto; omega.
  - rewrite (filter_none _ Hops7).
    exists (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      (length (mops_move (C' + t, t) (R' + x, t) t v) + 1))); simpl.
    split; [repeat rewrite app_length; simpl; omega|].
    rewrite firstn_app, firstn_length', minus_plus, firstn_app, firstn_length',
      minus_plus; auto; omega.
  - exploit t_steps_move_n; try apply Hmove; try apply split_in; auto.
    intros ((v & n' & Hle & Hlcr) & ?).
    exists v, 0, (S (length (mops_hb_check (W' + x) (C' + t) vs1 vs2 zt t) +
      n')); simpl.
    rewrite firstn_app, firstn_length', minus_plus; [|omega].
    ...
    + rewrite <- Nat.sub_0_le in Hle.
      rewrite Hlcr, firstn_app; setoid_rewrite Hle.
      rewrite app_nil_r; auto.
  - exploit t_steps_hb_check_n; try apply Hcheck; try apply split_in; auto.
    intros ((vs1 & vs2 & n' & Hle & Hlcr) & ?).
    exists vs1, vs2, 0, 0, (S n'); simpl.
    rewrite <- Nat.sub_0_le in Hle; rewrite Hlcr, firstn_app, Hle.
    split; [|rewrite app_nil_r; auto].
    rewrite app_length; omega.
  - rewrite (filter_none _ Hops1); exists [], [], 0, 0, 1; clarify.
    clear; omega.
Qed.    
(* !! add abstract *)  

Lemma instrument_indep : forall P0 G0 t o c P G lo lc P1 G1 P2 G2 i rest
  (Hdistinct : distinct P0) P0' (Hsim : state_sim P0' P0)
  (Hsafe : safe_locs P0') (Hfresh : fresh_tmps P0')
  (Ht : Forall (fun e => fst e < zt) P0')
  (Hlocks : forall l, locks l P0 -> good_lock (l, 0) P0)
...
Proof.
  ...
  rewrite List.firstn_length, Min.min_l; auto; intro Ht_steps.
  rewrite app_assoc, firstn_skipn in Hin.
  destruct i; try destruct x0; clarify.
  - generalize (in_split _ _ Hin); clarify.
    exploit exec_next; eauto; clarify.
    exploit exec_keep; try apply Hsteps; eauto; clarify.
    destruct n.
    ...
    simpl in *; rewrite skipn_nil in *.
    exploit exec_maintain; try apply Hdistinct'; eauto; intro.
    exploit exec_minus_ops; eauto; intro Hnone.
    rewrite (filter_none _ Hnone), Forall_forall; auto.
  - 
...
Lemma noninterference : forall t P P0 (Hsim : state_sim P P0)
  (Hdistinct : distinct P0) (Hsafe : safe_locs P) (Hfresh : fresh_tmps P)
...
Proof.
  ...
  { exploit state_suffix_inv; eauto.
    ...
    exploit distinct_steps; eauto.
    { eapply exec_t_exec; eauto. }
    intro; assert (exists P0' G0' lo0' lc0',
      exec_star_t t (Some P0) G0 lo0 lc0 (Some P0') G0' /\
      exec_star_minus t (Some P0') G0' lo0' lc0' (Some P1) G1 /\
      Forall (fun c' => Forall (fun c1 => Forall (fun c2 =>
        ...
        (lc ++ opt_to_list c))) lo0' /\
      (forall t' li (Hdiff : t' <> t) (Hin : In (t', li) P1)
       (Horig : Forall (fun c' => VectorClocks.thread_of c' <> t') lo0'),
       exists li0, In (t', li0) P /\ li = instrument li0 t') /\
      consistent (m ++ lc0 ++ lc0' ++ lc ++ opt_to_list c))
      as (P0' & G0' & lo0' & lc0' & Hroot0 & Hroot' & Hsep & Hinstr & Hcon').
    { do 5 eexists; eauto.
    ...
    remember (Some P1) as Pa; remember (Some P2) as Pb; generalize dependent P1;
      generalize dependent lc0'; generalize dependent lo0'; induction Hsteps;
      clarify.
    { rewrite Forall_forall; auto. }
    destruct P'; [|inversion Hsteps].
    exploit exec_ops; eauto; intro Ht'.
    exploit exec_rd_ops; eauto; intro Ho.
    destruct (find (fun x => beq (VectorClocks.thread_of x) t'0) lo0')
      eqn: Hfind.
    { rewrite find_spec in Hfind; clarify.
      exploit nth_error_in; eauto; intro.
      rewrite Forall_forall in Hsep; exploit Hsep; eauto.
      unfold beq in Hfind21; clarify. }
    destruct (find (fun x => beq (VectorClocks.thread_of x) t') lo0')
      eqn: Hfind'.
    - destruct (eq_dec t'0 t'); clarify.
      ...
      { intros.
        ...
        + exploit in_split; eauto; clarify.
          ...
          { rewrite in_app; right; simpl; right; eauto. }
          intros ((?, ?) & ?); clarify.
          contradiction H52222; rewrite in_map_iff.
    ...
    - rewrite find_fail in *.
      ...
      destruct li0; clarify.
      exploit instrument_indep; try apply Hsim; eauto; try apply split_in.
      ...
      { eapply exec_star_trans; [eapply exec_t_exec | eapply exec_minus_exec];
          eauto. }
      { rewrite Forall_forall in Ht; exploit Ht; eauto. }
      intro X.
      use X; [|eapply exec_step_inv; try apply exec_minus_exec; try apply Hstep;
               eauto].
...
Lemma first_finished : forall P P0 (Hsim : state_sim P P0)
  (Hdistinct : distinct P0) t G0 P3 (Hsuffix : state_suffix P P3) 
  P2 G2 lo' lc' G3 (Hsteps2 : exec_star (Some P2) G2 lo' lc' (Some P3) G3)
  lo0 lc0 P1 G1 (Hsteps0 : exec_star_t t (Some P0) G0 lo0 lc0 (Some P1) G1)
...
Proof.
  ...
  - exploit state_suffix_inv; eauto.
    ...
    assert (Forall (fun c' => match c with Some a => loc_of c' <> loc_of a
      | None => True end) lc).
    { admit. (* This is where noninterference #2 comes in. *) }
    ...
    { exploit Forall2_in1; try apply H; eauto; intros ((?, ?) & ?); clarify.
      ...
      destruct (instr_result t (last (instrument_instr i t) (Lock 0)) (G1 t) v)
        as [((((?, ?), ?), ?), ?)|]; clarify.
      apply split_in. }
    exploit exec_t_iexec; try apply Hdistinct.
    { exploit Forall2_in1; try apply Hsim; eauto; intros ((?, ?) & ?); clarify;
        eauto. }
    ...
    intro; repeat rewrite <- app_assoc.
    do 2 rewrite (app_assoc m); apply loc_comm_ops_SC.
    { destruct c; clarify. }
    { eapply prog_step; eauto. }
    { eapply prog_steps, exec_minus_exec; eauto. }
  - destruct P'; [|inversion Hsteps2].
    specialize (IHHsteps2 _ eq_refl).
    destruct (eq_dec t0 t).
    + subst.
      exploit state_suffix_inv.
      { eauto. }
      { auto. }
      { eapply exec_star_trans; [eapply exec_t_exec | eapply exec_minus_exec];
          eauto. }
      ...
      { eapply consistent_app_SC.
        rewrite <- app_assoc in Hcon.
        do 3 rewrite app_assoc in Hcon.
      ...
      { auto. }
      { eauto. }
      { rewrite <- app_assoc, app_assoc in Hcon.
        rewrite loc_comm_ops_SC in Hcon.
        rewrite <- app_assoc, (app_assoc lc0) in Hcon; eauto.
        { eapply Forall_impl; eauto; clarify.
          destruct c; clarify. }
        { eapply prog_steps, exec_minus_exec; eauto. }
        { eapply prog_step; eauto. } }
    ...
    + exploit IHHsteps2; eauto 2.
      ...
      setoid_rewrite <- (app_assoc lc1 (opt_to_list c)) in Hm at 3.
      rewrite <- app_assoc; eauto.
Qed.
...
Proof.
  ...
  rewrite <- exec_rev in Hsteps.
  use IHHsteps; [|eapply consistent_app_SC; rewrite <- app_assoc; eauto].
  ...
  - exploit exec_result; eauto; intros (? & i & ? & ? & v & ? & Hresult).
    ...
    + right; rewrite skipn_app, not_le_minus_0 in *; [|omega].
      ...
      destruct l; clarify.
      exists t.
      (* We could use an open-ended instrument_indep, where we don't know that
...
Lemma exec_iexec : forall P P' (Hfinal : final_state (Some P')) G' G lo lc
  ...
  (Hinit_l : forall l, l < zl -> initialized m (l, 0))
  (Hinit_v : forall v, v < zv -> initialized m (X' + v, 0)),
  exists lo' lc', iexec_star P G lo' lc' P' G' /\ consistent (m ++ lc0 ++ lc').
  (* And not just an arbitrary lc'; one that can read the same values? *)
Proof.
  ...
  { clarify.
    ...
    specialize (H _ Hlt _ eq_refl _ _ _ Hexec').
    specialize (H (P1a ++ (t, li) :: P1b)); repeat use H.
    specialize (H _ Hdistinct0 _ Hsim0); clarify.
    exploit H; auto.
    - eapply exec_step_inv; eauto.
    - rewrite <- (app_assoc _ _ lc1); eauto.
    - clarify.
    ...
    - unfold fresh_tmps in *; rewrite Forall_app in *; clarify.
      inversion Hfresh2 as [|?? Hi]; inversion Hi; constructor; auto.
    - unfold safe_locs in *; rewrite Forall_app in *; clarify.
      inversion Hsafe2 as [|?? Hi]; inversion Hi; constructor; auto.
  ...
  { repeat rewrite filter_app.
    ...
    rewrite (filter_negb_none _ Hallt), (filter_negb_none _ Hallt'), app_nil_r; 
      simpl.
    rewrite <- app_assoc; repeat rewrite <- filter_app.
    setoid_rewrite Forall_app in Hsafe; setoid_rewrite Forall_app in Hfresh;
      clarify.
    inversion Hsafe2 as [|?? Hsafe_i]; inversion Hfresh2 as [|?? Hfresh_i];
      inversion Hsafe_i; inversion Hfresh_i; subst.
    eapply instrument_indep; try apply Hsim0; eauto.
    - exploit bounded_sim; try apply Hsim0; auto; intro.
      ...
      setoid_rewrite Forall_forall in Hbound; exploit Hbound;
        [rewrite in_app|]; clarify.
    - apply consistent_app_SC with (m2 := lc2).
      do 4 erewrite <- (app_assoc _ _ lc2); eauto. }
  ...
  destruct (instr_result t i (G t) 0) as [((((th, ?), ?), ?), ?)|] eqn: Hresult.
  specialize (H (P1a ++ (t, li) :: opt_to_list th ++ P1b)); repeat use H.
  specialize (H _ Hdistinct0 _ Hsim0); clarify.
  exploit H; eauto.
  { eapply exec_star_trans; eauto.
    eapply exec_t_exec; eauto. }
  { repeat rewrite filter_app.
    ...
    do 2 rewrite <- app_assoc; rewrite (filter_all _ Hallt),
      (filter_all _ Hallt'); auto.
    - setoid_rewrite Forall_app in Hsafe; setoid_rewrite Forall_app in Hfresh;
        clarify.
      inversion Hsafe2 as [|?? Hsafe_i]; inversion Hfresh2 as [|?? Hfresh_i];
        inversion Hsafe_i; inversion Hfresh_i; subst.
      eapply Forall_impl; [|eapply instrument_indep; try apply Hsim0; eauto].
      + simpl; intros.
        repeat rewrite filter_app, Forall_app in *; clarify.
      + exploit bounded_sim; try apply Hsim0; auto; intro.
        exploit bounded_steps; eauto; intro Hbound.
        setoid_rewrite Forall_forall in Hbound; exploit Hbound;
          [rewrite in_app|]; clarify.
      + apply consistent_app_SC with (m2 := lc2).
        do 4 erewrite <- (app_assoc _ _ lc2); eauto.
    - eapply prog_steps; eauto.
    - rewrite (filter_all _ Hallt'); eapply prog_step; eauto. }
  intros (lo' & lc' & ? & ?).
  repeat eexists; [eapply iexec_step; eauto|].
  repeat rewrite <- app_assoc in *; auto.
  - eapply fresh_result; eauto.
  - eapply safe_result; eauto.
  - assert (P1 = Pa ++ (t, instrument li t) ::
      map (fun e => (fst e, instrument (snd e) (fst e))) (opt_to_list th) ++ Pb) by admit. (* prove this! *)
...
Definition no_assert := instr_forall
...
Proof.
  ...
  { subst.
    ...
    - rewrite <- (app_assoc _ _ lc1); eauto.
    - clarify.
      rewrite <- app_assoc in *; eexists; exists (opt_to_list c ++ x0);
        repeat eexists; [eapply iexec_step| |]; eauto.
      exploit instrument_single; eauto; destruct i; clarify;
        inversion Hexec0; exploit distinct_thread; eauto; clarify.
      + apply iexec_assign; auto.
      + eapply iexec_assert; auto.
      + rewrite <- app_assoc; auto.
    - unfold no_asserts in *; rewrite Forall_app in *; clarify.
      inversion Hno_asserts2 as [|?? Hi]; inversion Hi; constructor; auto.
    - unfold fresh_tmps in *; rewrite Forall_app in *; clarify.
      inversion Hfresh2 as [|?? Hi]; inversion Hi; constructor; auto.
    - unfold safe_locs in *; rewrite Forall_app in *; clarify.
      inversion Hsafe2 as [|?? Hi]; inversion Hi; constructor; auto.
  ...
  - eapply component_decr; eauto.
    { eapply exec_t_exec; eauto. }
    rewrite app_length, Hinstr; simpl; omega.
  - inversion Hexec0; subst.
    exploit distinct_thread; eauto; intros (? & ? & Heq).
    exploit (instrument_incom (Assert_le e1 e2)); simpl; eauto; clarify.
...
Proof.
  ...
  -rename P into P2, P' into P2', P'' into P2'', G' into G2', G'' into G2''.
   ...
   { eapply distinct_steps; eauto.
     eapply iexec_exec; eauto. }
   assert(consistent ((m ++ c2)++ lc2)) as Hcon_mlc2.
   { rewrite <-app_assoc. auto. }
   assert( forall p : ptr, meta_loc p -> initialized (m++c2) p) as Hinit_P2'.
   { clarify. apply init_steps; auto. apply iexec_exec in Hexec; auto. apply prog_steps in Hexec; auto. }
   ...
   specialize(IHHstep Hstate_sim_P1'_P2' (m++c2) Hinit_P2' Hcon_mlc2 Hclock_sim_s').
   inversion IHHstep as (lo1 & lc1 & P1''&G1''& Hexec_star_P1'_P1''&Hstate_sime_P1''_P2''
                             & Henv_sim_G1''_G2'' & Hcon_mc2_lc1 & Hmem_sim'_lc1_lc2
...
Proof. Print instrument_sim_safe.
  ...
  -exists []. exists []. exists P2. exists G2. 
   split;[|split;[|split;[|split]]]; clarify.
   +apply exec_refl.
   +admit.
   +admit.
  -admit.
...
Lemma state_sim_final : forall P1 P2
  (Hstate_sim : state_sim P1 P2) (Hfinal_P2: final_state (Some P2)),
                          final_state (Some P1).
Proof.
  clarify.
...
Theorem instrument_correct : forall P (Hsafe_locs: safe_locs (init_state P))
  ...
  (exists ops2 m2 P2' G2', exec_star (Some (init_state (instrument P 0)))
     ...
     consistent (m0 ++ m2)) <-> (exists ops m P' G',
   exec_star (Some (init_state P)) init_env ops m (Some P') G' /\
   final_state (Some P') /\ exists s, step_star s0 ops s).
Proof.
  intros. split.
  -(*completeness*) (* i.e., instrumented execution -> race-free *)
   ...
   exploit exec_iexec'; eauto. 
    ...
    eapply state_sim_final; eauto.
  -(*soundness*)
   intros (ops & m & P' & G' & Hexec_star_orig & Hfinal_P' & Hsafe).
   ...
   { unfold init_state. rewrite Forall_forall. clarify. omega. }
   { admit. } (*need to add as an assumption*)
   { unfold init_state, state_sim. clarify. }
   { do 2 instantiate (1:=init_env). unfold env_sim. clarify. }
   { do 2 instantiate(1:=[]). instantiate(1:= P). apply exec_refl. }
   { apply Hexec_star_orig. }
   { simpl. admit. } (* need to add to the right of the existentials*)
   { instantiate(1:=s0). unfold clocks_sim. admit. } (* conflict: the initial memory in the original program is [], while that of the instrumented program is m0*)
