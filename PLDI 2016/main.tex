...
\newcommand{\VC}{v}
\newcommand{\VCFont}{}
\newcommand{\SWState}{\VCFont{V}}
%\newcommand{\SWState}{\heartsuit}
...
\newcommand{\VCHB}{\sqsubseteq}
\newcommand{\VCJoin}[2]{#1 \sqcup #2}
...
\newcommand{\EpochPairs}{\textit{EpochPair}s\xspace}
\newcommand{\EpochVC}{\textit{EpochPlusVC}\xspace}
\newcommand{\EpochVCs}{\textit{EpochPlusVC}s\xspace}
\newcommand{\Rule}[1]{\textsc{#1}}
\newcommand{\CurWEpocht}{\mathbb{W}_t}
\newcommand{\VCMax}{\sqcup}
\newcommand{\VCCompare}{\sqsubseteq} 
\newcommand{\EpochCompare}{\preceq}
%\newcommand{\EmptyEpoch}{\bot_e} % TODO: not used
...
precedes $b$ in synchronization order, \eg, a lock release $\RelOp{\Tid}{\Lock}$
and subsequent acquire $\AcqOp{\TidU}{\Lock}$; or (3) $(a,b)$ is in the
...
%If $a$ happens before $b$ then we also say that $b$ \emph{happens after} $a$.
...
execution \cite{fidge-vc,mattern-vc}.  A vector clock $\VC$ stores one integer
logical clock per thread.  There are two key binary operations on vector clocks:
\emph{union} is the element-wise maximum of two vector clocks
($\VCJoin{\VC_1}{\VC_2} = \VC_3 \SuchThat \forall \Tid \Bind \VC_3(\Tid) =
\max(\VC_1(\Tid), \VC_2(\Tid))$); \emph{happens-before} is the element-wise
comparison of two vector clocks ($\VC_a \VCHB \VC_b$ is defined to mean $\forall
\Tid \Bind \VC_a(\Tid) \leq \VC_b(\Tid)$).  A vector-clock race detector keeps
four kinds of state:
\begin{itemize}
...
and $\forall \TidU \neq \Tid \Bind \ThreadVC{\Tid}(\TidU) = 0$.  On a lock
acquire $\AcqOp{\Tid}{\Lock}$, we update $\ThreadVC{\Tid}$ to
$\VCJoin{\ThreadVC{\Tid}}{\LockVC{\Lock}}$.  By acquiring lock $\Lock$, thread
...
first check if $\WriteVC{\Address} \VCHB \ThreadVC{\Tid}$.  If this
...
\VCHB \ThreadVC{\Tid}$ and $\ReadVC{\Address} \VCHB \ThreadVC{\Tid}$.  If
...
\emph{Epochs} can often be used in place of vector clocks; an epoch $c@t$ is a reduced vector clock that holds a timestamp for just one thread, and is treated as a vector clock that is $c$ for $t$ and 0 for every thread other than $t$. Because epochs have a single non-zero entry, an epoch can be compared with a vector clock, or another epoch, in \Constant time using the $\EpochCompare$ operator. We say $c@t \EpochCompare V$ when $c \leq V(t)$.
%, and $c@t \EpochCompare c'@u$ when $t = u$ and $c \leq c'$. % jld: defn doesn't handle 0@t0 <= 1@t0. Probably not worth specifying this precisely.
$\bot_e$ denotes a minimal epoch $0@t0$, though other minimal epochs exist, \eg, $0@t1$.

The \FT analysis state is a tuple $(C, L, M)$, where $C_t$ is a vector clock for the thread $t$, $L_m$ is a vector clock for the lock $m$, and $M_x = (M^R_x, M^W_x)$ is a read vector clock/epoch and a write epoch for the location $x$. \FT's initial analysis state is the tuple $(\lambda t.inc_t(\bot_V), \lambda l.\bot_V , \lambda x. (\bot_e,\bot_e))$. Each thread initially has an empty vector clock, with its own entry incremented, all locks have empty vector clocks, and all memory locations have empty read and write epochs. 

Intuitively, $C_u(t)$ records the last timestamp at which the thread $t$ synchronized with $u$, and $L_m(t)$, $M^R_x(t)$, and $M^W_x(t)$ record the last timestamps at which $t$ accessed the lock $m$, read from the location $x$, and wrote to the location $x$, respectively. The write metadata for a location is always an epoch; the read metadata may be either an epoch or a full vector clock, depending on whether the location is being read concurrently by multiple threads. We write \EpochPair for the set of location metadata pairs $(R, W)$ in which both $R$ and $W$ are epochs, and \EpochVC for the set of pairs in which $R$ is a vector clock and $W$ is an epoch. The \FT operational semantics are presented in the left columns of \autoref{f:semft}.

\TODO{are these necessary here? Would be good to avoid them I suppose}
We have made three small departures from the \FT semantics as originally presented. First, we represent metadata for a location $x$ as a pair of read and write metadata $M_x$, instead of tracking read metadata and write metadata in separate pieces of analysis state, to more closely mirror our implementation. We also extend the \FT \Rule{WriteExclusive} rule (\autoref{f:semft}) to reset the read epoch $M'^R_x := \bot_e$. This extension is correct because $M'^W_x := E(t)$ and $M^R_x \EpochCompare E(t)$. Any subsequent race-free access to $x$ must be well-ordered with respect to $M'^W_x$ and thus must also be well-ordered with $M^R_x$. Any subsequent racing access to $x$ that is concurrent with $M^R_x$ will be concurrent with $M'^W_x$ as well, so the read epoch is extraneous and can be cleared. Finally, we have added a \Rule{ReadSharedSameEpoch} rule which is used when a thread $t$ reads \EpochVC metadata multiple times within a given epoch. This lets us distinguish cases where \EpochVC updates are necessary, which have different semantics with \TODO{drop this?}.
...
\bigcell{
\inference[\Rule{ReadSameEpoch}]{M^R_x = E(t)}{(C, L, M) \Rightarrow^{\mathit{rd}(t, x)} (C, L, M)}\\\\

\inference[\Rule{ReadSharedSameEpoch}]
{M_x \in \mathit{\EpochVC} \\ M^W_x \EpochCompare C_t \\ M^R_x[t] = C_t(t)}
{(C, L, M) \Rightarrow^{\mathit{rd}(t, x)} (C, L, M)}\\\\

\inference[\Rule{\Rule{ReadShared}}]{M_x \in \mathit{\EpochVC} \\ M^W_x \EpochCompare C_t \\ M' = M[x := (M^R_x[t := C_t(t)], M^W_x) ] }{(C, L, M) \Rightarrow^{\mathit{rd}(t, x)} (C, L, M')}\\\\

\inference[\Rule{\Rule{ReadInflate}}]{M_x \in \mathit{\EpochPair} \\ M^W_x \EpochCompare C_t \\ M^R_x = c@u \\ V = \bot_V[t := C_t(t), u := c] \\ M' = M[x := (V,M^W_x)]}{(C, L, M) \Rightarrow^{\mathit{rd}(t, x)} (C, L, M')}\\\\

\inference[\Rule{\Rule{ReadExclusive}}]{M_x \in \mathit{\EpochPair} \\ M^R_x \EpochCompare C_t \qquad M^W_x \EpochCompare C_t \\ M' = M[x := (E(t),M^W_x)] }{(C, L, M) \Rightarrow^{\mathit{rd}(t, x)} (C, L, M')}
} & &
\bigcell{
\inference[\Rule{WriteSameEpoch}]{M^W_x = E(t)}{(C, L, M) \Rightarrow^{\mathit{wr}(t, x)} (C, L, M)}\\\\

\inference[\Rule{\Rule{WriteExclusive}}]{M_x \in \mathit{\EpochPair} \\ M^R_x \EpochCompare C_t \qquad M^W_x \EpochCompare C_t \\ M' = M[x := (\bot_e,E(t))] }{ (C, L, M) \Rightarrow^{\mathit{wr}(t, x)} (C, L, M')}\\\\

\inference[\Rule{\Rule{WriteShared}}]{M_x \in \mathit{\EpochVC} \\ M^R_x \VCCompare C_t \qquad M^W_x \EpochCompare C_t \\ M' = M[x := (\bot_e,E(t))] }{ (C, L, M) \Rightarrow^{\mathit{wr}(t, x)} (C, L, M') }\\\\

\inference[\Rule{\Rule{Fork}}]{C' = C[u := C_u \VCMax C_t, t := inc_t(C_t)]}{(C, L, M) \Rightarrow^{\mathit{fork}(t, u)} (C', L, M)}\\\\

\inference[\Rule{Join}]{C' = C[t := C_t \VCMax C_u, u := inc_u(C_u)]}{(C, L, M) \Rightarrow^{\mathit{join}(t, u)} (C', L, M)}\\\\

\inference[\Rule{Acquire}]{C' = C[t := (C_t \VCMax L_m)]}{(C, L, M) \Rightarrow^{\mathit{acq}(t, m)} (C, L, M)}\\\\

\inference[\Rule{\Rule{Release}}]{L' = L[m := C_t] \\ C' = C[t := inc_t(C_t)]}{(C, L, M) \Rightarrow^{\mathit{rel}(t, m)} (C', L', M)}
}
\end{tabular}
\caption{\FT operational semantics.}
\label{f:semft}
\end{figure*}
