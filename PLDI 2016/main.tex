...
\newcommand{\execstar}[4]{\ensuremath{(#1, #2) \tto{#3} #4}}
\newcommand{\iexec}[4]{\ensuremath{(#1, #2) \Rightarrow_{#3} #4}}
\newcommand{\execstart}[4]{\ensuremath{(#1, #2) \rightarrow^{*}_{#3} #4}}
\newcommand{\execstarm}[4]{\ensuremath{(#1, #2) \rightarrow^{*}_{\not #3} #4}}
...
execution \cite{vcfidge,vcmattern}.  A vector clock $\VC$ stores one (non-negative) integer
...
\begin{figure}[htb]
\bigcell{
\inference{}{\iexec{P \uplus (t, \assign{a}{e}\texttt{;} \mathit{li}, G)}{m}{t}{(P \uplus (t, \mathit{li}, G[a \mapsto \mathsf{eval}(G, e)]), m)}}\\\\

\inference{\mathsf{read}(m, p) = v \quad \forall o.\ (W + p, o) \le (C + t, o) \\\mathsf{read}(m, X + p) = 0 \quad m' = \mathsf{write}(m, (C + t, t), (R + x, t))}{\iexec{P \uplus (t, \instr{t}{\load{a}{p}}\texttt{;} \mathit{li}, G)}{m}{t}{(P \uplus (t, \mathit{li}, G[a \mapsto v, \texttt{tmp1} \mapsto ?, \texttt{tmp2} \mapsto ?]), m')}}
}
\TODO{How can we present this?}
\caption{Bigger-step semantics of instrumented programs}
\label{iexec}
\end{figure}
Key to our correctness proof is the idea that the instrumented program can be thought of as executing under a bigger-step semantics in which each instruction and its instrumentation execute in a single step. This semantics is shown in Figure~\ref{iexec}.
