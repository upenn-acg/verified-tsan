...
where by abuse of notation we use $K^b(t)$ to refer to the $C_u$ component of $\sigma'$ updated as appropriate for a blocking operation. (Proof?) With this statement of the lemma, the completeness proof follows as outlined in the paper.

We can also prove correctness of FastTrack by relating it to the already-verified vector clock algorithm. (How much detail for this? It uses a slightly modified VC, and while the proof is 20\% shorter, it's also somewhat harder to understand.)
...
Key to our correctness proof is the idea that the instrumented program can be thought of as executing under a bigger-step semantics in which each instruction and its instrumentation execute in a single step. This semantics is shown in Figure~?
