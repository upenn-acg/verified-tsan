...
logical clock per thread.  There are three key operations on vector clocks.
...
\emph{Epochs} can often be used in place of vector clocks; an epoch $c@t$ is a reduced vector clock that holds a timestamp for just one thread, and is treated as a vector clock that is $c$ for $t$ and 0 for every thread other than $t$. Because epochs have a single non-zero entry, an epoch can be compared with a vector clock, or another epoch, in \Constant time using the $\EpochCompare$ operator. We say $c@t \EpochCompare V$ when $c \leq V(t)$ \TODO{iff other entries of V are zero, right?}, and $c@t \EpochCompare c'@u$ when $t = u$ and $c \leq c'$. \TODO{jld: defn doesn't handle 0@t0 <= 1@t0. Should specify this precisely.}
...
\end{tabular}
\caption{An example of how \FT's write epoch encapsulates the conventional write vector clock.}
...
$\WriteOp{0}{\Address}{}$ \\
\end{tabular}
\caption{An example of how \FT's write epoch encapsulates a conventional read vector clock.}
...
& & $\ReadOp{2}{\Address}{}$ \\
\end{tabular}
\caption{An example of how \FT's read vector clock partially encapsulates the conventional read vector clock.}
