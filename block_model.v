...
Section Simple.
  ...
  Lemma consistent_proj : forall (m : list _) b (Hcon : consistent m)
    (Hread : read_init m) (Hwrite : write_alloc m),
    consistent (proj_block m b).
  Proof.
    induction m using rev_ind; clarify.
    rewrite consistent_proj_snoc in Hcon; clarify.
    destruct (eq_dec (block_of x) b).
    - rewrite proj_block_app; unfold proj_block in *; clarify.
    - rewrite proj_block_app; unfold proj_block, beq in *; clarsimp.
      apply IHm; auto.
  ...
  Proof.
    ...
    generalize (consistent_proj _ (block_of x) Hcon); clarify.
  ...
  Proof.
    ...
    rewrite proj_block_app, exclude_block_app in *; clarify.
      ...
      generalize (consistent_proj _ (block_of x) H2); intro Hcon.
  ...
  Proof.
    ...
    - clarify.
      ...
      use IHm.
      rewrite consistent_core; clarify.
      generalize (consistent_proj _ (block_of x) Hcon1); intro H.
      destruct (not_read x) eqn: Hnot; clarify.
      { rewrite proj_block_app, proj_filter_comm in H; clarify; apply H.
        { apply read_init_filter'; auto. }
        { apply write_alloc_filter'; auto. } }
      use H; [|apply read_init_filter'; clarsimp; eapply read_init_app; eauto].
      use H; [|apply write_alloc_filter'; clarsimp; eapply write_alloc_app;
               eauto].
